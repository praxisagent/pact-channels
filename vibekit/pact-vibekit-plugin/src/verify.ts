...
const PACT_STATUS_NAMES: Record<number, string> = {
  0: 'Active',
  1: 'WorkSubmitted',
  2: 'Approved',
  3: 'Disputed',
...
export function decideVerification(args: {
  ...
  const base: SubmitWorkVerifiedResult = {
    verified: false,
    pact_id: args.pactId,
  ...
  if (
    args.statusCode !== PactStatus.WorkSubmitted &&
    ...
    return {
      ...base,
      reason:
        'pact_not_in_verifiable_state: status=' +
        statusName +
        ' (code ' +
        args.statusCode +
        ')',
...
export async function submitWorkVerified(params: {
  ...
  const onChainHash: string = (pact.workHash ?? ZERO_HASH).toLowerCase();
  ...
  let decision: SwornDecisionFetched;
  try {
