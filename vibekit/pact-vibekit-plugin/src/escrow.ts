...
import {
  ...
  DEFAULT_ARBITRATION_WINDOW,
} from './constants.js';
import type { UnsignedTx, TxPlanResponse, EscrowState } from './types.js';
...
export function buildCreateEscrow(params: {
  recipient: string;
  amountPact: string;       // in PACT (e.g. "100" for 100 PACT)
  arbitrator?: string;       // defaults to zero address (no arbitrator)
  arbitratorFeePact?: string; // defaults to "0"
  deadlineUnix: number;      // unix timestamp
  disputeWindowSec?: number; // defaults to 24h
  arbitrationWindowSec?: number; // defaults to 72h
}): TxPlanResponse {
...
export function buildSubmitWork(params: {
  pactId: string;
  workContent: string; // arbitrary string — SHA-256 computed here
}): TxPlanResponse & { workHash: string } {
 ...
 */
export function buildApprovePayment(params: { pactId: string }): TxPlanResponse {
  const tx: UnsignedTx = {
    to: PACT_ESCROW_V2,
 ...
 */
export function buildReclaimEscrow(params: { pactId: string }): TxPlanResponse {
  const tx: UnsignedTx = {
    to: PACT_ESCROW_V2,
