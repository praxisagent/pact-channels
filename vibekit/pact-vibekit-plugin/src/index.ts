...
const TOOLS: Tool[] = [
  {
    name: 'create_escrow',
    description:
      'Build unsigned transactions to create a PACT escrow. Returns an ERC-20 approve tx and an escrow create tx. The caller must sign and submit both in order. The pactId is emitted in the PactCreated event.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    name: 'submit_work',
    description:
      'Build an unsigned transaction to submit work for a PACT escrow. SHA-256 hashes the workContent and submits the hash on-chain. Keep the original workContent to prove delivery.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description:
      'Build an unsigned transaction to approve and release payment for a completed PACT escrow. Must be called by the escrow creator after work is submitted.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    name: 'reclaim_escrow',
    description:
      'Build an unsigned transaction to reclaim PACT from an expired escrow where no work was submitted. Must be called by the creator after the deadline.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description:
      'Read the current state of a PACT escrow from Arbitrum One. Returns creator, recipient, amount, deadline, status, and whether payment is releaseable.',
    inputSchema: {
      type: 'object',
      properties: {
        pactId: { type: 'string', description: 'The escrow pact ID' },
  ...
  },
  {
    name: 'open_channel',
    description:
      'Build unsigned transactions to open a PACT payment channel with another agent. Returns an ERC-20 approve tx and a channel open tx. Enables unlimited off-chain micropayments with only 2 on-chain transactions total.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    name: 'send_payment',
    description:
      'Compute the EIP-712 typed data hash for an off-chain PACT payment update. Returns the hash and structured data for the agent to sign. Both parties must sign for a valid update. No on-chain transaction needed.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    name: 'close_channel',
    description:
      'Build an unsigned transaction for cooperative close of a PACT payment channel. Both agents must have signed the final state (use send_payment to get the hash, then both sign it).',
    inputSchema: {
      type: 'object',
      properties: {
        channelId: { type: 'string', description: 'The payment channel ID' },
  ...
  },
  {
    ...
    description:
      'Read the current state of a PACT payment channel from Arbitrum One.',
    inputSchema: {
      type: 'object',
      properties: {
        channelId: { type: 'string', description: 'The payment channel ID' },
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    switch (name) {
      case 'create_escrow': {
        const result = buildCreateEscrow(args as Parameters<typeof buildCreateEscrow>[0]);
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
      }

      case 'submit_work': {
        const result = buildSubmitWork(args as Parameters<typeof buildSubmitWork>[0]);
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
      }

      case 'approve_payment': {
        const result = buildApprovePayment(args as Parameters<typeof buildApprovePayment>[0]);
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
      }

      case 'reclaim_escrow': {
        const result = buildReclaimEscrow(args as Parameters<typeof buildReclaimEscrow>[0]);
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
      }

      case 'get_escrow_status': {
        const result = await getEscrowStatus({
          pactId: (args as { pactId: string }).pactId,
          rpcUrl: RPC_URL,
        });
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
      }
      ...
      case 'open_channel': {
        const result = buildOpenChannel(args as Parameters<typeof buildOpenChannel>[0]);
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
      }

      case 'send_payment': {
        const result = buildPaymentUpdateHash({
          ...(args as { channelId: string; nonce: number; balanceA: string; balanceB: string }),
          inPact: true,
        });
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
      }
      ...
      case 'close_channel': {
        const result = buildCoopClose({
          ...(args as Parameters<typeof buildCoopClose>[0]),
          inPact: true,
        });
        return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
      }
      ...
      case 'submit_work_verified': {
        const a = args as { pactId: string; manifestUrl: string; fetchTimeoutMs?: number };
